  ...
  return (
    <div style={{ padding: "40px", display: "flex", flexDirection: "column", gap: 32, overflowY: "auto", height: "100%" }}>
      <div className="reveal">
         ...
         <p style={{ color: "var(--text-dim)", marginTop: 4 }}>Analyzing development metrics for {activePlot.name}</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 24 }}>
        <div className="premium-card reveal" style={{ animationDelay: "0.1s" }}>
           <div className="text-label">Projected Revenue Yield</div>
           <div className="text-value" style={{ marginBottom: 32 }}>
              ₹{(revenueEst / 1000).toFixed(0)}k <span style={{ color: "var(--accent-secondary)", fontSize: 14 }}>+12.4% vs Baseline</span>
           </div>
        ...
        <div className="premium-card reveal" style={{ animationDelay: "0.2s", display: "flex", flexDirection: "column", gap: 24 }}>
           <div>
              <div className="text-label">Harvest Countdown</div>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12 }}>
                 ...
                 <div className="text-value" style={{ fontSize: 24 }}>{daysToHarvest} <span style={{ fontSize: 12, color: "var(--text-ghost)" }}>DAYS LEFT</span></div>
              </div>
           </div>

           <div>
              <div className="text-label">Development Progress</div>
           ...
           <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ padding: 16, borderRadius: 12, background: "var(--bg-surface)" }}>
                 <div className="text-label" style={{ fontSize: 9 }}>HEALTH</div>
                 <div style={{ fontSize: 16, fontWeight: 800 }}>Optimal</div>
              </div>
              <div style={{ padding: 16, borderRadius: 12, background: "var(--bg-surface)" }}>
                 <div className="text-label" style={{ fontSize: 9 }}>STABILITY</div>
                 <div style={{ fontSize: 16, fontWeight: 800 }}>Scale 9.4</div>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="reveal" style={{ animationDelay: "0.3s" }}>
         <div className="text-label" style={{ marginBottom: 16 }}>Growth Sequence</div>
                  ...
                  <div key={i} className="premium-card" style={{ 
                    ...
                    opacity: isDone ? 0.6 : 1
                     ...
                     <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                        <span style={{ fontSize: 10, fontWeight: 900, color: "var(--text-ghost)" }}>PHASE 0{i+1}</span>
                        {isDone && <ShieldCheck size={14} color="var(--accent-secondary)" />}
                     </div>
                     <div style={{ fontSize: 16, fontWeight: 800 }}>{stage.name}</div>
                     <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 4 }}>Day {stage.startDay}-{stage.endDay}</div>
                  </div>
               );
            })}
         </div>
