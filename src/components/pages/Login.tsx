    ...
    <div style={{
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
          {/* Segmented Control */}
          <div style={{ display: "flex", background: "rgba(0,0,0,0.04)", borderRadius: 100, padding: 4 }}>
            <button onClick={() => setMode("signup")} style={{ padding: "8px 20px", borderRadius: 100, background: mode === "signup" ? "#FFF" : "transparent", boxShadow: mode === "signup" ? "0 2px 8px rgba(0,0,0,0.06)" : "none", fontSize: 13, fontWeight: 700, color: mode === "signup" ? "var(--text-main)" : "var(--text-ghost)", transition: "all 0.2s", border: "none", cursor: "pointer" }}>{tr("Sign up", language)}</button>
            <button onClick={() => setMode("signin")} style={{ padding: "8px 20px", borderRadius: 100, background: mode === "signin" ? "#FFF" : "transparent", boxShadow: mode === "signin" ? "0 2px 8px rgba(0,0,0,0.06)" : "none", fontSize: 13, fontWeight: 700, color: mode === "signin" ? "var(--text-main)" : "var(--text-ghost)", transition: "all 0.2s", border: "none", cursor: "pointer" }}>{tr("Sign in", language)}</button>
          </div>
          <button style={{ width: 34, height: 34, borderRadius: "50%", background: "rgba(0,0,0,0.04)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-ghost)", border: "none", cursor: "pointer", transition: "background 0.2s" }} onMouseOver={e => e.currentTarget.style.background = "rgba(0,0,0,0.08)"} onMouseOut={e => e.currentTarget.style.background = "rgba(0,0,0,0.04)"}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {mode === "signup" && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ display: "flex", gap: 12 }}>
              <input placeholder={tr("First name", language)} style={{ ...inputStyle, padding: "16px" }} onFocus={e => { e.currentTarget.style.background = "#FFF"; e.currentTarget.style.borderColor = "var(--accent-primary)"; }} onBlur={e => { e.currentTarget.style.background = "rgba(255, 255, 255, 0.6)"; e.currentTarget.style.borderColor = "rgba(0, 0, 0, 0.05)"; }} />
              <input placeholder={tr("Last name", language)} style={{ ...inputStyle, padding: "16px" }} onFocus={e => { e.currentTarget.style.background = "#FFF"; e.currentTarget.style.borderColor = "var(--accent-primary)"; }} onBlur={e => { e.currentTarget.style.background = "rgba(255, 255, 255, 0.6)"; e.currentTarget.style.borderColor = "rgba(0, 0, 0, 0.05)"; }} />
            </motion.div>
          )}
          
          <div style={{ position: "relative" }}>
            <Mail size={18} color="var(--text-ghost)" style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)" }} />
            <input type="email" value={mode === "signin" ? email : ""} onChange={e => setEmail(e.target.value)} placeholder={tr("Enter your email", language)} style={inputStyle} onFocus={e => { e.currentTarget.style.background = "#FFF"; e.currentTarget.style.borderColor = "var(--accent-primary)"; }} onBlur={e => { e.currentTarget.style.background = "rgba(255, 255, 255, 0.6)"; e.currentTarget.style.borderColor = "rgba(0, 0, 0, 0.05)"; }} />
          </div>

          <div style={{ position: "relative" }}>
            <Lock size={18} color="var(--text-ghost)" style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)" }} />
            <input type="password" value={mode === "signin" ? password : ""} onChange={e => setPassword(e.target.value)} placeholder={tr("Password", language)} style={inputStyle} onFocus={e => { e.currentTarget.style.background = "#FFF"; e.currentTarget.style.borderColor = "var(--accent-primary)"; }} onBlur={e => { e.currentTarget.style.background = "rgba(255, 255, 255, 0.6)"; e.currentTarget.style.borderColor = "rgba(0, 0, 0, 0.05)"; }} />
          </div>
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={loading} type="submit" style={{ width: "100%", padding: 18, background: "var(--text-main)", color: "#FFF", borderRadius: 14, fontSize: 15, fontWeight: 700, border: "none", cursor: "pointer", marginTop: 8, boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} style={{ flex: 1, padding: 16, background: "rgba(255,255,255,0.8)", border: "1px solid rgba(0,0,0,0.05)", borderRadius: 14, cursor: "pointer", display: "flex", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.02)" }}>
